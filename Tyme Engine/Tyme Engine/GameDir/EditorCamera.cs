...
namespace Tyme_Engine
{
    class EditorCamera : UserScript
    {
        ...
        private void ConsoleCommands()
        {
            ...
            {
                foreach ( Component comp in parentObject.childComponents)
                {
                    if (comp.GetType() == typeof(CameraComponent))
                    {
                        Debug.Log("SettingFOV");
                        ((CameraComponent)comp).SetFieldOfView(45.0f);
                    }
            ...
            if (Keyboard.IsKeyPressed(Keys.O))
            {
                ...
                {
                    if (comp.GetType() == typeof(CameraComponent))
                        ((CameraComponent)comp).SetFieldOfView(90.0f);
                }
            }
            if (Keyboard.IsKeyPressed(Keys.P))
            {
                ...
                {
                    if (comp.GetType() == typeof(CameraComponent))
                        ((CameraComponent)comp).SetFieldOfView(120.0f);
                }
            }
        }
    }
